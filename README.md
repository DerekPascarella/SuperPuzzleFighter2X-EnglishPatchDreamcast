...
This patch leaves the secret "extra" options menu functional (accessible by holding L+R buttons when selecting "OPTIONS" after the title screen).  Rather than ubiquitously hardcoding English throughout the game, the default language setting is changed from Japanese to English.  Doing so permits players to change between additional supported languages if they so choose.
<br><br>
Leaving the language selection menu in-tact, however, can also prevent English text from being displayed under a certain condition.
<br><br>
